...
class Sentinel2Discriminator(nn.Module):
    """
    PatchGAN discriminator for Sentinel-2 imagery with residual connections.
    """
    def __init__(self, config, disc_type='vnir'):
        super(Sentinel2Discriminator, self).__init__()
        self.config = config
        self.disc_type = disc_type.lower()
        
        # Determine input channels based on discriminator type
        if self.disc_type == 'vnir':
            self.input_channels = config.vnir_channels
            self.input_resolution = config.vnir_resolution
            print(f"Creating VNIR discriminator for {self.input_resolution}×{self.input_resolution} input with {self.input_channels} channels")
        elif self.disc_type == 'swir':
            self.input_channels = config.swir_channels
            self.input_resolution = config.swir_resolution
            print(f"Creating SWIR discriminator for {self.input_resolution}×{self.input_resolution} input with {self.input_channels} channels")
        else:
            raise ValueError(f"Unknown discriminator type: {disc_type}. Use 'vnir' or 'swir'.")
        ...
        print(f"Using {self.n_layers} downsampling layers for {self.disc_type} discriminator")
            ...
            for j in range(2):  # Add 2 residual blocks at each level
                res_blocks.append(
                    ResidualDiscriminatorBlock(
                        ...
                        block_name=f"{disc_type}_layer{i+1}_res{j+1}"  # Add meaningful block names
                    )
                )
    ...
    def forward(self, x, angles=None):
        ...
        # Print input shape
        print(f"\n{self.disc_type.upper()} Discriminator forward pass:")
